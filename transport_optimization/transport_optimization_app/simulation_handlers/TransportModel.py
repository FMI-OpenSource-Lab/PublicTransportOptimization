...
class TransportModel(Model):
    ...
    def __create_passengers(self):
        ...
        for i in range(0, self.num_passengers * len(self.__times), len(self.__times)):
    ...
    def __fill_routes_to_graph(self):
        ...
        for route, stops in self.routes_solution.items():
            for i in range(len(stops) - 1):
                ...
                for j in range(len(self.__times)):
                    ...
                    if travel_time and distance:
                        attr_name = f"score_{j}"
                        self.__time_to_graph_attr_map[str(self.__times[j])] = attr_name
                        self.__graph.add_edge(stop1, stop2, {attr_name: travel_time + distance})
    ...
    def __get_transfers_recursive(self, path, routes_to_check, current_stop):
        ...
        for route in routes_to_check:
            ...
            for i in range(current_stop_index + 1, len(path)):
                next_stop = path[i]
                current_route_stops = self.routes_solution[current_route]
                if next_stop in current_route_stops \
                    ...
                    continue
                transfers += 1
                possible_next_routes = [r for r in self.routes_solution.keys()
                                        if current_stop in self.routes_solution[r]
                                        and next_stop in self.routes_solution[r]
                                        and abs(self.routes_solution[r].index(current_stop)
                                                - self.routes_solution[r].index(next_stop)) == 1]
    ...
    def get_transfers_count_in_route(self, path):
        """Get the number of transfers in a path"""
        possible_start_routes = [r for r in self.routes_solution.keys()
                                 if path[0] in self.routes_solution[r]
                                 and path[1] in self.routes_solution[r]
                                 and abs(self.routes_solution[r].index(path[0])
                                         - self.routes_solution[r].index(path[1])) == 1]
    ...
    def find_best_route(self, start_stop, end_stop, time_of_the_day):
        ...
        routes = list(self.routes_solution.keys())
        direct_routes = [r for r in routes if start_stop in self.routes_solution[r]
                         and end_stop in self.routes_solution[r]]

        if direct_routes:
            best_route = min(direct_routes, key=lambda r: sum(self.__get_travel_time_and_distance(r,
                                                                                                  start_stop,
                                                                                                  end_stop,
                                                                                                  time_of_the_day)))
        ...
        transfers_count = self.get_transfers_count_in_route(best_route)
        ...
        return min_transfers, best_time, best_distance
